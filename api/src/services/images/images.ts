...
interface ImagesByDeviceParams {
  deviceId: string
}

export const imagesByDevice = ({ deviceId }: ImagesByDeviceParams) => {
...
export const Image = {
  device: (_obj, { root }: ResolverArgs<ReturnType<typeof image>>) =>
    db.image.findUnique({ where: { id: root.id } }).device(),
}
